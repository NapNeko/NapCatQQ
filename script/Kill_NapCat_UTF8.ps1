...
# 获取 NapCat 主进程的 PID
$mainProcesses = Get-Process | Where-Object { $_.MainWindowTitle -match "^管理员:  NapCat|^Administrator:  NapCat" }
if (-not $mainProcesses) {
    Write-Output "未找到任何 NapCat 主进程"
    exit 1
}
...
foreach ($mainProcess in $mainProcesses) {
    ...
    Write-Output "已结束当前的 NapCat 主进程"
}
