...
var ApprovalService = class {
  ...
  async _autoApprove(groupId, userId, flag, comment, ttl) {
  ...
  async _autoReject(groupId, userId, flag, comment, reason, ttl) {
...
var lru = new LRUCache({ max: 2e3, ttl: 3e5 });
...
var ttlCache = new LRUCache({ max: 500, ttl: 6e4 });
...
async function boot(ctx) {
  ...
  const candidates = [
    { abs: join4(pluginPath, "dist", "webui", "index.html"), rel: "dist/webui/index.html" },
    // pre-built (zip install)
    { abs: join4(pluginPath, "webui", "index.html"), rel: "webui/index.html" }
    // source (git clone without build)
  ];
  const found = candidates.find((c) => existsSync3(c.abs));
  if (found) {
    ctx.router.static("/static", dirname2(found.rel));
    ctx.router.page({
      path: "guardian",
      title: "QQ Guardian",
      icon: "\u{1F6E1}\uFE0F",
    ...
    log.info({ htmlFile: found.rel }, "WebUI registered via ctx.router");
  } else {
...
async function plugin_cleanup(_ctx2) {
  await teardown();
}
...
async function plugin_get_config(_ctx2) {
  try {
    const cfg = configManager.get();
    return {
...
async function plugin_set_config(_ctx2, config) {
  if (!configManager.get()) return;
  const c = config;
  configManager.update({
    core: { selfId: c["selfId"] !== void 0 ? Number(c["selfId"]) : void 0 },
    approval: { defaultAction: c["defaultApproval"] ?? void 0 },
    risk: {
      enabled: c["riskEnabled"] !== void 0 ? Boolean(c["riskEnabled"]) : void 0,
      threshold: c["riskThreshold"] !== void 0 ? Number(c["riskThreshold"]) : void 0,
      action: c["riskAction"] ?? void 0
    },
    blacklist: { autoKickOnJoin: c["autoKickBlacklisted"] !== void 0 ? Boolean(c["autoKickBlacklisted"]) : void 0 }
  });
}
export {
  plugin_cleanup,
