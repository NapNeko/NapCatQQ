...
export abstract class HttpServerBase {
  ...
  logger: LogWrapper;
  serverToken: string | undefined = undefined;
  ...
  constructor(logger: LogWrapper, token: string) {
    ...
    this.expressAPP.use((req, res, next) => {
      ...
      originalJson(req, res, (err) => {
        if (err) {
          this.logger.logError('Error parsing JSON:', err);
  ...
  authorize(req: Request, res: Response, next: () => void) {
    ...
    if (this.serverToken && clientToken != this.serverToken) {
  ...
  start(port: number, host: string) {
    try {
      ...
      this.logger.logError('HTTP服务启动失败', e.toString());
  ...
  registerRouter(method: 'post' | 'get' | string, url: string, handler: RegisterHandler) {
    ...
    this.expressAPP[method](url, this.authorize, async (req: Request, res: Response) => {
      ...
      this.logger.logDebug('收到http请求', url, payload);
      try {
  ...
  protected listen(port: number, host: string = '0.0.0.0') {
    ...
    try {
      this.server = this.expressAPP.listen(port, host, () => {
        ...
        this.logger.logError('HTTP服务启动失败', err.toString());
      });
    } catch (e: any) {
      this.logger.logError('HTTP服务启动失败, 请检查监听的ip地址和端口', e.stack.toString());
    }
  }
}
