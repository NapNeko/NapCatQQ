...
export async function boot(ctx: NapCatPluginContext): Promise<void> {
  ...
  // ctx.router.static()/page() expect paths RELATIVE to pluginPath (confirmed via
  // official docs: https://napneko.github.io/develop/plugin/start-webui uses
  // ctx.router.static('/static', 'webui/dist') and htmlFile: 'webui/dist/index.html').
  // We validate existence using the absolute form, but register the relative form.
  const candidates = [
    { abs: join(pluginPath, 'dist', 'webui', 'index.html'), rel: 'dist/webui/index.html' }, // pre-built (zip install)
    { abs: join(pluginPath, 'webui', 'index.html'),          rel: 'webui/index.html' },      // source (git clone without build)
  ];
  const found = candidates.find(c => existsSync(c.abs));

  if (found) {
    ctx.router.static('/static', dirname(found.rel));
    ctx.router.page({
      path: 'guardian',
      title: 'QQ Guardian',
      icon: '🛡️',
