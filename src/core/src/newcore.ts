import QQWrapper, { NodeIQQNTWrapperEngine, NodeIQQNTWrapperSession, NodeQQNTWrapperUtil, WrapperNodeApi } from '@/core/wrapper';
...
export enum NewCoreWorkMode {
  NoInit = 0,
  Shell = 1,
  LiteLoader = 2
}
export class NewNapCatCore {

  public session: NodeIQQNTWrapperSession | undefined;
  public util: NodeQQNTWrapperUtil | undefined;
  public engine: NodeIQQNTWrapperEngine | undefined;
  public loginListener: LoginListener | undefined;
  private loginService: NodeIKernelLoginService | undefined;
  private onLoginSuccessFuncList: OnLoginSuccess[] = [];
  public workMode: NewCoreWorkMode = NewCoreWorkMode.NoInit;
  ...
  get dataPath(): string {
    if (!this.util) throw new Error('util not init');
  ...
  private initDataListener() {
    ...
    if (!requireMinNTQQBuild('26702')) {
      if (!this.session) throw new Error('session not init');
  ...
  private initConfig() {
    if (!this.engine) throw new Error('engine not init');
    if (!this.loginService) throw new Error('loginService not init');
