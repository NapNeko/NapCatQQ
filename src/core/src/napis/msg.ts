...
export class NTQQMsgApi {
  ...
  async getMsgEmojiLikesList(peer: Peer, msgSeq: string, emojiId: string, emojiType: string, count: number = 20) {
    ...
    return this.context.core.session.getMsgService().getMsgEmojiLikesList(peer, msgSeq, emojiId, emojiType, "", false, 20)
  }
  ...
  async setEmojiLike(peer: Peer, msgSeq: string, emojiId: string, set: boolean = true) {
    ...
    return this.context.core.session.getMsgService().setMsgEmojiLikes(peer, msgSeq, emojiId, emojiId.length > 3 ? '2' : '1', set);
  }
  async getMultiMsg(peer: Peer, rootMsgId: string, parentMsgId: string): Promise<GeneralCallResult & {
    msgList: RawMessage[]
  } | undefined> {
    return this.context.core.session.getMsgService().getMultiMsg(peer, rootMsgId, parentMsgId);
  }
  async ForwardMsg(peer: Peer, msgIds: string[]) {
    return this.context.core.session.getMsgService().forwardMsg(msgIds, peer, [peer], new Map());
  }
  ...
  async getMsgsByMsgId(peer: Peer | undefined, msgIds: string[] | undefined) {
    ...
    return await this.context.core.session.getMsgService().getMsgsByMsgId(peer, msgIds);
  }
  async getSingleMsg(peer: Peer, seq: string) {
    return await this.context.core.session.getMsgService().getSingleMsg(peer, seq);
  }
  async fetchFavEmojiList(num: number) {
    return this.context.core.session.getMsgService().fetchFavEmojiList("", num, true, true)
  }
  async queryMsgsWithFilterExWithSeq(peer: Peer, msgSeq: string) {
    let ret = await this.context.core.session.getMsgService().queryMsgsWithFilterEx('0', '0', msgSeq, {
  ...
  async getMsgsBySeqAndCount(peer: Peer, seq: string, count: number, desc: boolean, z: boolean) {
    return await this.context.core.session.getMsgService().getMsgsBySeqAndCount(peer, seq, count, desc, z);
  }
  async setMsgRead(peer: Peer) {
    return this.context.core.session.getMsgService().setMsgRead(peer);
  }
  async getGroupFileList(GroupCode: string, params: GetFileListParam) {
    let data = await this.context.event.CallNormalEvent<
  ...
  async getMsgHistory(peer: Peer, msgId: string, count: number, isReverseOrder: boolean = false) {
    // 消息时间从旧到新
    return this.context.core.session.getMsgService().getMsgsIncludeSelf(peer, msgId, count, isReverseOrder);
  }
  async recallMsg(peer: Peer, msgIds: string[]) {
    await this.context.core.session.getMsgService().recallMsg({
  ...
  async getMsgUnique(chatType: number, time: string) {
    if (requireMinNTQQBuild('26702')) {
      return this.context.core.session.getMsgService().generateMsgUniqueId(chatType, time);
    }
    return this.context.core.session.getMsgService().getMsgUniqueId(time);
  }
  async getServerTime() {
    return this.context.core.session.getMSFService().getServerTime();
  }
  async getServerTimeV2() {
    return this.context.event.CallNoListenerEvent<() => string>('NodeIKernelMsgService/getServerTime', 5000);
  }
  async forwardMsg(srcPeer: Peer, destPeer: Peer, msgIds: string[]) {
    return this.context.core.session.getMsgService().forwardMsg(msgIds, srcPeer, [destPeer], new Map());
  }
