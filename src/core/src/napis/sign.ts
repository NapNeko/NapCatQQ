...
export class NTQQWebApi {
  ...
  async SignMiniApp(CardData: MiniAppLuaJsonType) {
    ...
    //   "ver": "0.0.0.1"
    // };
    let data = (await this.context.core.getApiUser().getQzoneCookies());
  ...
  async SignMusicWrapper(id: string = '') {
    let MusicInfo = await this.CreateMusicThridWay0(id)!;
    let MusicCard = await this.SignMusicInternal(MusicInfo.name!, MusicInfo.singer!, MusicInfo.cover!, MusicInfo.mid!, "https://ws.stream.qqmusic.qq.com/" + MusicInfo.url!);
    return MusicCard;
  }
}