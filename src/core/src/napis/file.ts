...
import { GeneralCallResult, OnRichMediaDownloadCompleteParams } from '@/core';
...
export class NTQQFileApi {
  ...
  async copyFile(filePath: string, destPath: string) {
    await this.context.core.util.copyFile(filePath, destPath);
  }

  async getFileSize(filePath: string): Promise<number> {
    return await this.context.core.util.getFileSize(filePath);
  }
  async getVideoUrl(peer: Peer, msgId: string, elementId: string) {
    return (await this.context.core.session.getRichMediaService().getVideoPlayUrlV2(peer, msgId, elementId, 0, { downSourceType: 1, triggerType: 1 })).urlResult.domainUrl;
  }
  // 上传文件到QQ的文件夹
  async uploadFile(filePath: string, elementType: ElementType = ElementType.PIC, elementSubType: number = 0) {
    // this.context.core.wrapper.util.
    ...
    const mediaPath = this.context.core.session.getMsgService().getRichMediaFilePathForGuild({
      md5HexStr: fileMd5,
      fileName: fileName,
  ...
  async downloadMediaByUuid() {
    //this.context.core.session.getRichMediaService().downloadFileForFileUuid();
  }
  ...
  async addFileCache(peer: Peer, msgId: string, msgSeq: string, senderUid: string, elemId: string, elemType: string, fileSize: string, fileName: string) {
    ...
    return this.context.core.session.getSearchService().addSearchHistory({
      type: 4,
      contactList: [],
      id: -1,
      groupInfos: [],
  ...
  async searchfile(keys: string[]) {
    ...
    const Event = this.context.event.CreatEventFunction<EventType>('NodeIKernelSearchService/searchFileWithKeywords');
    let id = '';
    const Listener = this.context.event.RegisterListen<(params: OnListener) => void>('NodeIKernelSearchListener/onSearchFileKeywordsResult', 1, 20000, (params) => {
...
export class NTQQFileCacheApi {
  ...
  clearCache(cacheKeys: Array<string> = ['tmp', 'hotUpdate']) {
    // 参数未验证
    return this.context.core.session.getStorageCleanService().clearCacheDataByKeys(cacheKeys);
  }

  addCacheScannedPaths(pathMap: object = {}) {
    return this.context.core.session.getStorageCleanService().addCacheScanedPaths(pathMap);
  }
  ...
  scanCache(): Promise<GeneralCallResult & {
    ...
    return this.context.core.session.getStorageCleanService().scanCache();
  }
  ...
  getChatCacheList(type: ChatType, pageSize: number = 1000, pageIndex: number = 0) {
    return this.context.core.session.getStorageCleanService().getChatCacheInfo(type, pageSize, 1, pageIndex);
  }

  getFileCacheInfo(fileType: CacheFileType, pageSize: number = 1000, lastRecord?: CacheFileListItem) {
    const _lastRecord = lastRecord ? lastRecord : { fileType: fileType };
    //需要五个参数
    //return this.context.core.session.getStorageCleanService().getFileCacheInfo();
  }

  async clearChatCache(chats: ChatCacheListItemBasic[] = [], fileKeys: string[] = []) {
    return this.context.core.session.getStorageCleanService().clearChatCacheInfo(chats, fileKeys);
  }
}
